...
class Character_Management(OpenRTM_aist.DataFlowComponentBase):
    ...
    def onExecute(self, ec_id):
        if self._coordinateIn.isNew():
            ...
            print("Received coordinate:", data.data)
            self._processed_coordinate.data = data.data
            self._processed_coordinateOut.write()

        if self._imageIn.isNew():
            data = self._imageIn.read()
            print("Received image:", data.data)
            self._processed_image.data = data.data
            self._processed_imageOut.write()

        if self._voiceIn.isNew():
            data = self._voiceIn.read()
            print("Received voice:", data.data)
            self._processed_voice.data = data.data
            self._processed_voiceOut.write()
